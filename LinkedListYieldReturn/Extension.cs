...
namespace LinkedListYieldReturn
{
    public static class Extension
    {
        ...
        public static LinkList<Tout> Select<Tin, Tout>(this LinkList<Tin> list, Func<Tin, Tout> expression)
        {
            LinkList<Tout> results = new LinkList<Tout>();
        ...
        public static LinkList<T> OrderBy<T, Tkey>(this LinkList<T> list, Func<T, Tkey> keySelector)
            where Tkey : IComparable
        {
            if (list.GetLength() == 0)
                return new LinkList<T>();

            // Convert LinkList into array O(n)
            ...
            for (int i = 0; i < array.Length; i++)
            {
                for (int j = i; j > 0; j--)
                {
                    if (keySelector.Invoke(array[j])
                        .CompareTo(keySelector.Invoke(array[j - 1])) < 0)
                    {
            ...
            // Conver array back to LinkList O(n)
            LinkList<T> result = new LinkList<T>();
            for (int i = 0; i < array.Length; i++)
            {
                result.Add(array[i]);
